...
NOT_EVEN_SYMBOL = '.'
ROW_LENGTH = len(input1[0])
RE_DIGIT = r'\d+'
RE_SYMBOL = r'[^\w\s.]'


def day3part1():
    dummy_row = NOT_EVEN_SYMBOL * ROW_LENGTH

    sum = 0
    for i, row in enumerate(input1):
        first_row = i == 0
        last_row = i == len(input1) - 1
        row_above = dummy_row if first_row else input1[i - 1]
        row_above = 'x' + row_above + 'x'
        row = 'x' + row + 'x'
        row_below = dummy_row if last_row else input1[i + 1]
